...
public class View implements IView {
	...
	private final Map<String, IMediator> mediatorMap;
	// Mapping of Notification names to Observer lists
	private final Map<String, Observers> observerMap;

	/**
	 * Constructor.
	 * 
	 * <P>
	 * This <code>IView</code> implementation is a Singleton, so you should not
	...
	public View() {
		...
		this.observerMap = new HashMap<String, Observers>();
		initializeView();
	}
	 ...
	 * 
	 * @param note
	 *            the <code>Notification</code> to notify <code>Observers</code>
	 *            of.
	 */
	public void notifyObservers(INotification note) {
		Observers observers = this.observerMap.get(note.getName());
		if (observers != null) {
	 ...
	 * </P>
	 * <P>
	 * If the <code>Mediator</code> returns any <code>Notification</code> names
	 * to be notified about, an <code>Observer</code> is created encapsulating
	 * the <code>Mediator</code> instance's <code>handleNotification</code>
	 * method and registering it as an <code>Observer</code> for all
	...
	public void registerObserver(String notificationName, IObserver observer) {
		if (this.observerMap.get(notificationName) != null) {
			this.observerMap.get(notificationName).addObserver(observer);
		} else {
	...
	public IMediator removeMediator(String mediatorName) {
		...
		// zero, remove it.
		IMediator m = retrieveMediator(mediatorName);

		for (Iterator<Observers> its = this.observerMap.values().iterator(); its.hasNext(); ) {
			Observers o = its.next();
			boolean retaining = false;
			for (Iterator<IObserver> it = o.iterator(); it.hasNext(); ) {
				IObserver i = it.next();
				if (i.compareNotifyContext(m)) {
					it.remove();
				} else {
					retaining = true;
				}
	 ...
	 * @return the <code>Mediator</code> instance previously registered with the
	 *         given <code>mediatorName</code>.
	 */
