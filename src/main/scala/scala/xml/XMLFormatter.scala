 ...
 */
class XMLFormatter(width: Int, step: Int) {
  ...
  /** By default, invokes attributes.buildString */
...
//        }
    ...
    minimizeTags: Boolean = false): StringBuilder =
  {
    x match {
      ...
      case _  =>
        ...
        if (x.attributes ne null) formatAttributes(x, sb)
    ...
    minimizeTags: Boolean = false): Unit =
  {
    ...
    else if (children forall Utility.isAtomAndNotText) { // add space
